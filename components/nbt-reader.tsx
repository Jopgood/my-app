...
class NBTReader {
  ...
  readTag() {
    ...
    console.log("Reading tag type:", type);
    if (type === 0) return null; // TAG_End

    if (!(type in TAG_TYPES)) {
      console.log("Current buffer position:", this.offset);
      console.log(
        "Next few bytes:",
        Array.from(
          new Uint8Array(this.buffer.slice(this.offset, this.offset + 8))
        )
...
const NBTFileReader = ({ onDataParsed }) => {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      setNbtData(data);
      // Notify parent component of the parsed data
      if (onDataParsed) {
        onDataParsed(data);
      }
    } catch (err) {
